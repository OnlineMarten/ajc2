...
<div class="row">
        <div v-show="selection.total_amount>0" class="col-sm-12">
            <hr>
            <h4>Payment</h4>
            <div class="alert alert-danger alert-dismissible fade show" v-if="show_error" role="alert" id="alert" name="alert">
                <button type="button" class="close" v-on:click="show = !show" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                {{ error }}
...
export default {
  ...
  data(){
    ...
    mounted() {
        ...
        let result = params.get("resultCode");
        ...
        makePayment(data){
            ...
            axios.post("/makepayment",{
                ...
                if( response.data.hasOwnProperty('action')) {
                    //we need to go to get additional details
                    this.additionalDetails(response.data['action']);
                }

                else{
                  //we get here after a payment attempt with a credit card.
                  //payment through ideal will be redirected and then send to the adyencontroller checkout function.
                  //first we check our response from adyen to see if we need to show the result page
                  //or stay on the booking page for a possible retry, so the client does not have to fill in the card details again
                  ...
                  switch (response.data.resultCode) {
                    ...
                    case "Authorised":
                      ...
                      console.log(response.data);
                    window.location = process.env.MIX_APP_URL+"/checkout?resultCode="+response.data.resultCode;//get the root folder from the .env file
                      break;
                    ...
                    case "Received":
                      // code block
                      console.log('Received returned, which means Pending');
                      //extend basket lifetime and notify admin
                      axios
                        .get("/extendbasketlifetime")
                        .then(response => {
                            console.log('basket lifetime extended');
                        })
                        .catch(error => {
                           //extending basket lifetime failed, notify admin
                           console.log('extending basket lifetime FAILED');
                        });
                //      window.location = process.env.MIX_APP_URL+"/checkout?resultCode="+response.data.resultCode;
                      break;

                    case "Error":
                      this.error = "The payment failed, we could not contact your bank. You can retry with the same card."
                      this.show_error = true;
                    ...
                    default:
                      console.log('Unknown status (from bookingcomponent)');
                      this.error = "Unknown error. We have been notified. Please contact us for more information"
                      this.show_error = true;
                  }
